...
<div role="none" class="wrapper" onclick={()=>closeable && dialogManager.close(onCloseData)}>
	<section role="none" onclick={e=>e.stopPropagation()}>
		{#if title}
			<header>
				<span>{title}</span>
				{#if closeable}
					<i role="none" class="close fa-fw fas fa-times" onclick={() => dialogManager.close(onCloseData)}></i>
				{/if}
			</header>
		{/if}
		<article role="none" onclick={e=>e.stopPropagation()}>
			{#if children}
				{@render children()}
			{/if}
		</article>
		{#if buttons}
			<footer role="none" onclick={e=>e.stopPropagation()}>
				{@render buttons()}
			</footer>
		{/if}
	</section>
